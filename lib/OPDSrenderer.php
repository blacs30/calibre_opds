...
class OPDSRenderer {
    ...
    public function getOpenSearch() {
        ...
        $xml->writeAttribute("template", Config::get('concos_full_url', '') . 'index.php' . $urlparam);
        $xml->endElement();
    ...
    private function startXmlDocument($page) {
        ...
        if (Config::get('concos_generate_invalid_opds_stream', 'false') === 'false' || preg_match("/(MantanoReader|FBReader)/", $_SERVER['HTTP_USER_AGENT'])) {
            // Good and compliant way of handling search
            $urlparam = addURLParameter($urlparam, "page", Base::PAGE_OPENSEARCH);
            $link = new Link("index.php" . $urlparam, "application/opensearchdescription+xml", "search", "Search here");
        } else {
    ...
    private function renderEntry($entry) {
        ...
        foreach ($entry->book->getAuthors() as $author) {
            ...
            self::getXmlStream()->endElement();
            self::getXmlStream()->startElement("uri");
            self::getXmlStream()->text("index.php" . $author->getUri());
            self::getXmlStream()->endElement();
            self::getXmlStream()->endElement();
        }
    ...
    public function render($page) {
        ...
        if ($page->isPaginated()) {
            ...
            self::getXmlStream()->text(Config::get('concos_max_item_per_page', '-1'));
            self::getXmlStream()->endElement();
            self::getXmlStream()->startElement("opensearch:startIndex");
            self::getXmlStream()->text(($page->n - 1) * (Config::get('concos_max_item_per_page', '-1') + 1));
            self::getXmlStream()->endElement();
